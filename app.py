...
def render_correlation_analysis(data: pd.DataFrame) -> None:
    ...
    with heatmap_col:
        st.plotly_chart(create_correlation_heatmap(correlation_matrix), use_container_width=True)
    with insight_col:
        st.markdown(
            f"""
            <div class="insight-card">
...
def main() -> None:
    ...
    st.caption("Source: historical University of Bradford station CSV.")
    st.plotly_chart(create_daily_conditions_chart(filtered_data), use_container_width=True)
