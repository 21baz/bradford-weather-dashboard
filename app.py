...
def main() -> None:
    ...
    if comparison.empty:
        if is_temperature_metric(selected_comparison_metric):
            st.error(
                "Bradford MET temperature comparison data could not be read. "
                "Expected monthly temperature columns in `data/met_data.csv` named like "
                "`tas January`, `tas February`, ..., `tas December` "
                "or `temperature January` / `temp January` equivalents."
            )
        else:
            st.info(
                "Bradford MET comparison is currently only available for temperature. "
                "Other metrics require matching MET baseline data."
            )
    else:
